...
class BoundaryTest(unittest.TestCase):
    def test_a_fresh_stream_is_waiting_for_a_command(self):
        self.assertTrue(protocol.Shape().at_boundary)

    def test_a_command_that_wants_bytes_leaves_the_stream_mid_command(self):
        self.assertFalse(feed(protocol.Shape(), protocol.MULTIPLY).at_boundary)

    def test_and_the_stream_is_back_at_a_boundary_once_it_has_them(self):
        shape = feed(protocol.Shape(), protocol.MULTIPLY, 2, 0, 3, 0)

        self.assertFalse(shape.at_boundary)
        self.assertEqual(shape.produced, 4)

    def test_a_sync_takes_nothing_and_leaves_the_stream_where_it_was(self):
        shape = feed(protocol.Shape(), protocol.SYNC)

        self.assertTrue(shape.at_boundary)
        self.assertEqual(shape.produced, 0)

    def test_a_byte_the_part_does_not_recognise_is_treated_the_same_way(self):
...
class ExpectingTest(unittest.TestCase):
    def test_a_fresh_stream_expects_a_command_rather_than_data(self):
        self.assertFalse(protocol.Shape().expecting_input)

    def test_a_command_mid_flight_is_still_expecting_its_data(self):
        self.assertTrue(feed(protocol.Shape(), protocol.TILE).expecting_input)

    def test_and_stops_expecting_once_it_has_it(self):
        self.assertFalse(feed(protocol.Shape(), protocol.TILE, *range(32)).expecting_input)


class OutputTest(unittest.TestCase):
    def test_a_tile_conversion_takes_thirty_two_bytes_and_gives_thirty_two_back(self):
        shape = feed(protocol.Shape(), protocol.TILE, *range(32))

        self.assertEqual(shape.produced, 32)

    def test_a_multiply_takes_four_and_gives_four(self):
        self.assertEqual(feed(protocol.Shape(), protocol.MULTIPLY, 1, 0, 2, 0).produced, 4)

    def test_a_transparent_colour_gives_nothing_back(self):
        self.assertEqual(feed(protocol.Shape(), protocol.TRANSPARENT, 0x0A).produced, 0)

    def test_a_merge_takes_twice_its_length_and_gives_its_length_back(self):
        shape = feed(protocol.Shape(), protocol.MERGE, 4, *range(8))

        self.assertEqual(shape.produced, 4)

    def test_a_mirror_takes_its_length_and_gives_its_length_back(self):
        shape = feed(protocol.Shape(), protocol.MIRROR, 4, *range(4))

        self.assertEqual(shape.produced, 4)

    def test_a_scale_gives_back_the_second_of_its_two_lengths(self):
        shape = feed(protocol.Shape(), protocol.SCALE, 4, 6, *range(2))

        self.assertEqual(shape.produced, 6)

    def test_a_merge_of_nothing_produces_nothing_and_waits_for_the_next_command(self):
...
class ReadingTest(unittest.TestCase):
    def test_reading_a_result_spends_it(self):
    ...
    def test_a_result_read_only_in_part_still_has_the_rest(self):
        shape = feed(protocol.Shape(), protocol.MULTIPLY, 1, 0, 2, 0)
    ...
    def test_reading_when_there_is_nothing_to_read_is_not_an_error(self):
    ...
    def test_a_stream_is_only_at_a_boundary_once_the_result_is_spent(self):
        shape = feed(protocol.Shape(), protocol.MULTIPLY, 1, 0, 2, 0)
...
class TransparentTest(unittest.TestCase):
    ...
    def test_a_fresh_stream_has_not_been_told_a_colour(self):
        self.assertIsNone(protocol.Shape().transparent)

    def test_the_colour_a_command_set_is_remembered(self):
        self.assertEqual(feed(protocol.Shape(), protocol.TRANSPARENT, 0x0A).transparent, 0x0A)

    def test_and_the_last_one_wins(self):
...
class ArmingTest(unittest.TestCase):
    ...
    def test_a_merge_length_given_with_no_payload_arms_the_next_merge(self):
        shape = feed(protocol.Shape(), protocol.MERGE, 0)
    ...
    def test_a_second_merge_after_a_length_runs_with_that_length(self):
