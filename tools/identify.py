...
def digests(data: bytes) -> Any:
    return Identity(**romimage.identity.measure(data))


def read_manifest(path: Path | str = MANIFEST_PATH) -> Any:
    return json.loads(Path(path).read_text())


def accepted_digests(manifest: Any) -> dict[str, Any]:
    return {
...
def run(manifest: Any, roms: Path = ROMS, wanted: tuple[str, ...] = ()) -> list[Any]:
    return [
