...
The second member whose output is a cartridge somebody plays, and the only one
that replaces a coprocessor with code rather than with data.

Dungeon Master is the one SNES cartridge carrying a DSP-2. That part converts the
game's Atari ST bitmaps into the console's tile format, composites them against a
transparent colour, mirrors and scales them, and multiplies. It is a rendering
helper rather than a decompressor, which is the fact the whole shape of this
project follows from: what it is asked to convert depends on where the player is
standing, so there is no fixed set of answers to compute ahead of time and store.
That was measured rather than assumed. Of the inputs the cartridge sends, every
one of the mirror command's is a run of bytes found in the retail dump, and 0.3%
of the tile command's are.

So the six operations are reimplemented in 65816 and the cartridge is pointed at
them. Correctness is settled against the cartridge's own recorded traffic: three
seeded tours of thirty thousand frames each, every byte in and out of the port
written down, replayed through the routines on a processor model that counts
cycles. Every command answers every recorded byte.

It consumes six other members as submodules at its own root. `snes-dsp` runs the
part's own microcode, which is what an expected answer comes from and is never
carried here; `mos65xx` is the processor the replacement runs on and the
instrument the replacement is measured with; `snes-mapper` and `snes-rom-image`
answer for the map and the image; `snes-graphics` answers for the tile format the
conversion produces; and `snes-driver` reads the cartridge's own code to find
where it reaches the part.

What this member has that the others do not is a cost gate. A chip worked while
the program that fed it carried on, so moving its work onto the processor cannot
be free, and the only honest way to talk about that is a number. `tools/cost.py`
runs each command on the processor model against real recorded traffic and prices
it against what the same exchange cost when a chip answered it. Both figures are
printed per command, because a single total would hide which one regressed.

Neither the cartridge nor the microcode is here, and neither is any patched
output. Everything published is a digest.
